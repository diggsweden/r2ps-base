...
public class StaticResources {
  ...
  static {
    ...
    PUBLIC_KEY_OBJECT_MAPPER = JsonMapper.builder()
        .serializationInclusion(JsonInclude.Include.NON_NULL)
        .addModule(createSimpleModule(true, false))
        .build();

    TIME_STAMP_SECONDS_MAPPER = JsonMapper.builder()
        .serializationInclusion(JsonInclude.Include.NON_NULL)
        .addModule(new JavaTimeModule())
        .configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, true)
        .configure(SerializationFeature.WRITE_DATE_TIMESTAMPS_AS_NANOSECONDS, false)
        .configure(DeserializationFeature.READ_DATE_TIMESTAMPS_AS_NANOSECONDS, false)
        .addModule(createSimpleModule(false, true))
        .build();

    SERVICE_EXCHANGE_OBJECT_MAPPER = JsonMapper.builder()
        .serializationInclusion(JsonInclude.Include.NON_NULL)
        .addModule(new JavaTimeModule())
        .configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, true)
        .configure(SerializationFeature.WRITE_DATE_TIMESTAMPS_AS_NANOSECONDS, false)
        .configure(DeserializationFeature.READ_DATE_TIMESTAMPS_AS_NANOSECONDS, false)
        .addModule(createSimpleModule(true, true))
        .build();
  }

  private static SimpleModule createSimpleModule(boolean publicKeySerialization,
      boolean instantSerialization) {
