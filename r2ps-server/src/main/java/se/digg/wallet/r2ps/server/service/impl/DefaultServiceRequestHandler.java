...
public class DefaultServiceRequestHandler implements ServiceRequestHandler {
  ...
  public DefaultServiceRequestHandler(OpaqueServiceRequestHandlerConfiguration configuration)
    ...
    this.encryptionMethod =
        configuration.getEncryptionMethod() == null
    ...
    try {
      ...
      final ServiceRequest serviceRequest =
          StaticResources.TIME_STAMP_SECONDS_MAPPER.readValue(
              jwsObject.getPayload().toString(), ServiceRequest.class);

      if (serviceRequest.getContext() == null) {
        throw new ServiceRequestHandlingException(
            "Not context is declared in the service request", ErrorCode.ILLEGAL_REQUEST_DATA);
      }

      if (replayChecker.isReplay(serviceRequest.getNonce())) {
        throw new ServiceRequestHandlingException(
            "The request is a replay", ErrorCode.ACCESS_DENIED);
      }
      ...
      if (clientPublicKeyRecord == null) {
        throw new ServiceRequestHandlingException(
            "No public key record available for this request", ErrorCode.ACCESS_DENIED);
      }
      ...
      if (!jwsObject.verify(verifier)) {
        throw new ServiceRequestHandlingException(
            "Invalid signature on service request", ErrorCode.ACCESS_DENIED);
      }
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "Server received service request with valid signature:\n{}",
      ...
      // Process request
      final ServiceTypeHandler serviceTypeHandler =
          serviceTypeHandlers.stream()
