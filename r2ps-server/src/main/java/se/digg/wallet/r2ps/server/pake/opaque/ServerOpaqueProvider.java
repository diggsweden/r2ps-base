...
public class ServerOpaqueProvider {
  ...
  public ServerOpaqueProvider(
      ...
      final Duration defaultMaxRecordDuration,
      final Duration finalizedDuration) {
  ...
  public EvaluationResponseResult evaluateAuthRequest(
      byte[] ke1, String clientId, String kid, String context)
  ...
  /**
   * Finalizes an authentication request by completing the PAKE (Password Authenticated Key
   * Exchange) session. This method validates the provided cryptographic proof and updates the
   * corresponding PAKE session record.
   *
  ...
  /**
   * Finalizes an authentication request by completing the PAKE (Password Authenticated Key
   * Exchange) session. This method validates the provided cryptographic proof, updates the PAKE
   * session record, and sets the session expiration time and task details.
   *
   * @param ke3 the cryptographic proof provided by the client to finalize the PAKE session
   * @param pakeSessionId the unique identifier of the PAKE session to finalize
  ...
  public FinalizeResponse finalizeAuthRequest(
      ...
      final Duration requestedSessionDuration)
      throws PakeSessionException, ClientAuthenticationException {
    ...
    try {
      ...
      Instant sessionExpirationTime =
          getSessionExpirationTime(sessionTask, requestedSessionDuration);
      ...
      return new FinalizeResponse(
          pakeSessionId, pakeSession.getExpirationTime(), pakeSession.getSessionTask());
  ...
  private Instant getSessionExpirationTime(
      final SessionTask sessionTask, final Duration requestedSessionDuration) {
    Duration maxDuration =
        sessionTask == null ? defaultMaxRecordDuration : sessionTask.maxDuration();
    if (maxDuration == null) {
      log.warn(
          "Session task {} does not have a max duration. Using default max duration {}",
          sessionTask,
    ...
    if (requestedSessionDuration == null) {
      log.debug(
          "Requested session duration is null. Setting session expiration time to max duration.");
